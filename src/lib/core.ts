...
import type { Options, HTTPOptions, Multiaddr } from '../types'
...
export const errorHandler = async (response: Response): Promise<undefined> => {
  let msg: string | null
...
const urlToString = (url: string | URL | Multiaddr | undefined): string => {
  if (url == null) {
    return ''
  } else if (url instanceof URL) {
    return url.toString()
  } else if (isMultiaddr(url)) {
    return url.toString()
  }
...
export class Client extends HTTP {
  ...
  constructor (options = {}) {
    ...
    this.fetch = (resource: string | Request, options: HTTPOptions = {}): any => {
      if (typeof resource === 'string' && !resource.startsWith('/')) {
        resource = `${urlToString(opts.url)}/${resource}`
      }
